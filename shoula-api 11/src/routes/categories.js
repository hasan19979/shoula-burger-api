...
router.get('/', asyncHandler(async (req, res) => {
  const result = await pool.query('SELECT id, slug, name, icon, sort_order, print_order FROM categories ORDER BY sort_order, id');
...
router.post('/', requireAuth, asyncHandler(async (req, res) => {
  ...
  const result = await pool.query(
    `INSERT INTO categories (slug, name, icon, sort_order, print_order) VALUES ($1, $2, $3, COALESCE($4, 0), COALESCE($5, 999)) RETURNING *`,
    [slug, name, icon || 'ti-circle-dot', sort_order, print_order]
  );
...
router.put('/:id', requireAuth, asyncHandler(async (req, res) => {
  ...
  const result = await pool.query(
    `UPDATE categories SET name = COALESCE($1, name), icon = COALESCE($2, icon), sort_order = COALESCE($3, sort_order), print_order = COALESCE($4, print_order)
     WHERE id = $5 RETURNING *`,
    [name, icon, sort_order, print_order, id]
  );
